...
import { ProductBlocks, ProductBlocksBuy_buttons, ProductBlocksComplementary, ProductBlocksDescription, ProductBlocksDynamic_buy_buttons, ProductBlocksImage, ProductBlocksInventory, ProductBlocksPayment_terms, ProductBlocksPre_order, ProductBlocksTitle, ProductBlocksVariant_selector } from "_types";
import { create, StoreApi, UseBoundStore } from "zustand";

const productSection = document.querySelector<HTMLElement>("[data-product-drawer-product-handle]");
const productHandle = productSection?.dataset?.productDrawerProductHandle ?? "";
const formId = productSection?.dataset?.productDrawerFormId ?? "";

export const useProductDrawer = create(initProductStore(productSection, productHandle, formId));
...
function ProductDrawerPreloadEvents({
  ...
  useEffect(() => {
    if (hydrated) {
      document.querySelectorAll<HTMLElement>("[data-drawer-product]").forEach((productElement) => {
        const productHandle = productElement.dataset.drawerProduct;
  ...
  const handleClick = useCallback((e) => {
    const disableCartOpen =
      e.target.closest("[data-drawer-product]")?.dataset?.productCardNoAutoCart;
    const spinner = e.currentTarget.querySelector("[data-loading-spinner]");
    spinner.classList.remove("opacity-0");

    const productHandle: string = e.target.closest("[data-product-drawer-add-button]")?.dataset
